...
# Or:
#   ./install.sh [--targets pi,claude,antigravity,codex,opencode,common,all] [--skills wiki-manager,constructor,all]
...
if [ -n "$CLI_TARGETS" ]; then
  if [ "$CLI_TARGETS" = "all" ]; then
    CHOSEN_TARGETS=("pi" "claude" "antigravity" "codex" "opencode" "common")
  else
